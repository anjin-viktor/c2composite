...
/**
Получение типа, к которому осуществится приведение при выполнении операции с операндами указанных типов в
языке СИ.
@param type1 - указатель на строковое представление 1-го типа
@param type2 - указатель на строковое представление 2-го типа 
@param buff - буфер для результата. Если buff == NULL то создается буфер подходящего размера.
В этом случае параметр n игнорируется
@param n - размер буфера для результата
@return - указатель на результат, NULL в случае отказа
*/
